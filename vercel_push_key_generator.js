...
fs.writeFileSync('vercel_push_key.js', `
const { execSync } = require('child_process');
execSync('npx vercel env rm GOOGLE_PRIVATE_KEY production -y');
execSync('npx vercel env add GOOGLE_PRIVATE_KEY production', { input: \`${key}\` });
`);
