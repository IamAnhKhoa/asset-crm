...
import { RefreshCw, CheckCircle, XCircle, Search, X } from 'lucide-react';
        ...
        <div className="flex-1 overflow-y-auto">
            <div className="sticky top-0 z-10 h-14 px-6 flex items-center justify-between bg-white/80 backdrop-blur-sm border-b border-slate-100">
                <div>
                    ...
                    <p className="section-subtitle">{loading ? '...' : `${filtered.length} phiếu ${tab === 'pending' ? 'chờ duyệt' : 'đã duyệt'}`}</p>
                </div>
                    ...
                    <table className="table">
                        <thead>
                            <tr>
                                ...
                                <th>Ghi chú</th>
                                {tab === 'pending' && <th className="text-center">Thao tác</th>}
                            </tr>
                        </thead>
                        <tbody>
                            {loading
                                ? Array.from({ length: 5 }).map((_, i) => (
                                    <tr key={i}>{Array.from({ length: tab === 'pending' ? 7 : 6 }).map((_, j) => <td key={j}><div className="skeleton h-4 rounded" /></td>)}</tr>
                                ))
                                : filtered.length === 0
